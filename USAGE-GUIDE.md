...
### Step 3-2 — (필수) 병렬 실행 모드 질문 — MCP 질문과 같은 메시지에서 함께 나온다
...
**Claude가 멈추고 응답을 기다리는 순간은 정확히 세 가지뿐이다** (Step 3의 MCP 질문과 Step 3-2의 병렬 모드 질문은 프로젝트 시작 시 1회성 절차라 이 목록과 별개다):
...
**멈추지 않고 알려주기만 하는 것**: 각 기능(feature)의 개발을 시작할 때 Claude가 "이 기능은 OO라 L__ 트랙으로, 이 단위들은 동시에 진행합니다"라고 통지한다. 응답하지 않아도 그대로 진행한다. 검증을 완화하는 빠른 트랙(L1·L2)은 사용자가 동의한 경우에만 쓰며, 동의가 없으면 기본 L3(정식 검증)로 간다. "사소한 기능은 L1로 가도 돼"처럼 미리 일괄 허용하면 이후 다시 묻지 않는다.
