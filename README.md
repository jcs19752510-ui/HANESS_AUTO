...
11. **서로 무관한 작업은 동시에 진행한다 (병렬 실행 모드 P0/P1/P2, 프로젝트 시작 시 1회 선택, 기본 P1).** 독립한 작업단위·feature, 08/09 검증 축, 11단계 문서 4종, 1단계 조사 축은 문서 근거로 독립성이 확인되면 사용자가 다시 요청하지 않아도 오케스트레이터가 동시에 실행한다. 병렬은 실행 순서만 바꾸며 규칙 B·C·D·F·K는 병렬 실행분에도 개별적으로 그대로 적용된다 (ORCHESTRATOR.md 1장 "병렬 실행 모드").
