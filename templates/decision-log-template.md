...
- **병렬 실행 모드(P0/P1/P2, ORCHESTRATOR.md 1장) 선택과 변경**은 사용자 응답으로 기록한다. 이후 각 병렬 계획(웨이브 구성, 순차로 뺀 단위와 사유)은 "에이전트 자체판단, 비가역성 Low"로 한 줄씩 기록하되, **판정 근거 문서 §**(02 §9, 03 작업단위 확정표 등)를 결정 내용에 반드시 적는다. 근거 없이 병렬/순차를 정한 기록은 무효다. 병렬 서브에이전트는 이 파일을 직접 수정하지 않으므로, 결과서의 "공유 문서 갱신 요청"을 받아 오케스트레이터가 일괄 기록한다.
