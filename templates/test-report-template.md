...
- 병렬 실행 정보 (ORCHESTRATOR.md 1장 "병렬 실행 모드" 참고): 단독 실행 / 병렬 웨이브 <번호>에서 실행(동시에 돌던 단위·feature·축 ID: ) / 조각(fragment) 결과를 취합한 최종 문서 (조각 경로: `docs/harness/_parallel/...`) 중 택1
- 테스트 목적: 
...
- 병렬 실행이었다면: 위 `git status`에 보이는 항목 중 다른 단위·feature·축 소유분과 그 소유자 표기 / "이 실행이 만든 임시 아티팩트·미추적 잔여물이 없음"이라는 판정 근거 / 웨이브 종료 후 오케스트레이터의 전체 트리 점검(`harness-janitor.sh --check`, 전체 `git status`) 결과 (취합 문서라면 모든 조각의 아티팩트·정리 결과를 합쳐 기재. 병렬이 아니었다면 "해당 없음(단독 실행)"):
