...
**취합 모드**(프롬프트에 "취합"이 지정된 호출): `_parallel/01-*.md` 조각을 모두 읽어 위 출력 계약의 8개 섹션을 갖춘 `01-trend-analysis.md`를 작성한다. 조각 간 상충하는 주장은 하나를 골라 숨기지 않고 상충 사실을 그대로 보고하며, 어느 조각도 다루지 못한 섹션은 "해당 없음(사유)"이나 "확인 필요"로 명시한다. 규칙 B의 내부 검증 2회와 `verify-log_01-trend-analysis.md`는 이 취합본 기준으로 수행한다. 축 지정 없이 호출되면 기존대로 단독으로 전 축을 조사한다.
