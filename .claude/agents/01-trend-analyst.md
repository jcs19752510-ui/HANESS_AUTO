...
# 병렬 처리 (P1 이상에서 기본 동작, ORCHESTRATOR.md 1장 "병렬 실행 모드"·4장 5-5 참고)
오케스트레이터가 조사 축(시장·사회 동향 / 경쟁 서비스 / 규제·법 / 도메인 운영규칙 / LLM·기술 이슈)별로 이 에이전트를 동시에 여러 번 호출할 수 있다. 호출 프롬프트에 **조사 축이 지정되면 조각 모드**로 동작한다:
- 지정된 축만 조사하고 결과를 `docs/harness/_parallel/01-<축>.md`에 쓴다. `docs/harness/01-trend-analysis.md`와 검증 로그는 직접 쓰지 않는다.
- 조각에도 모든 주장에 출처·조사 시점을 단다. 확인하지 못한 부분은 "확인 필요"로 명시한다. 이 축에서 발견한 다른 축 관련 단서는 버리지 말고 조각 하단 "타 축 참고" 절에 적는다.
- 규제 민감 도메인(규칙 I)·AI/LLM 기능(규칙 J) 조사 축을 맡았다면 아래 해당 절의 요구를 그 축 조각에서 빠짐없이 충족한다.
- 조각은 중간 산출물이라 규칙 B의 2회 검증은 하지 않는다(취합본 기준). 다만 출처 누락은 스스로 점검하고 반환한다.

**취합 모드**(프롬프트에 "취합"이 지정된 호출): `_parallel/01-*.md` 조각을 모두 읽어 위 출력 계약의 8개 섹션을 갖춘 `01-trend-analysis.md`를 작성한다. 조각 간 상충하는 주장은 하나를 골라 숨기지 않고 상충 사실을 그대로 보고하며, 어느 조각도 다루지 못한 섹션은 "해당 없음(사유)"이나 "확인 필요"로 명시한다. 규칙 B의 내부 검증 2회와 `verify-log_01-trend-analysis.md`는 이 취합본 기준으로 수행한다. **P1 이상에서는 축별 동시 호출이 기본값이며, 축 지정 없이 이 에이전트를 한 번만 호출하는 것은 병렬화 검토(Fan-out Checkpoint)를 빠뜨렸다는 신호다.** 축 지정 없이 호출되는 것은 사용자가 명시적으로 수동/단계별 모드를 요청했거나 P0(순차)를 선택한 경우에 한하며, 그 경우에만 단독으로 전 축을 조사한다.
