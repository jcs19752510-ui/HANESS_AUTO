...
- 결함 발견 시 5단계로 되돌려 수정 요청 (직접 코드를 고치지 않고 재작업을 요청하는 것을 원칙으로 하되, 사소한 오탈자 수준은 직접 수정 가능. 여기서 "사소한 오탈자 수준"이란 동작·로직·인터페이스(시그니처·응답 형식)를 바꾸지 않는 철자·주석·문자열 리터럴·포맷 수정으로서, 수정 파일 1개·변경 몇 줄 이내인 경우만 뜻한다. 직접 수정했다면 무엇을 어디서 고쳤는지 결과서 6절 결함 목록에 Fixed로 기록하고, 이 기준을 넘으면 5단계로 되돌린다.)
...
다른 작업 단위의 05·06 호출이 동시에 돌고 있다. 이 호출은 여전히 작업 단위 **하나**만 검증하며, 결과서의 엄격함(전 섹션, 규칙 B 2회 검증)은 단독 실행과 완전히 동일하다. 추가로:
- **임시 아티팩트 이름**은 반드시 이 단위 식별자를 포함한다(`.harness-tmp/venv_06_unit<n>/`, `.harness-tmp/db_06_unit<n>.sqlite3`). 서버 포트·DB·파일 경로는 다른 단위와 겹치지 않게 잡고, 고정 포트가 불가피하면 오케스트레이터에 보고해 이 단위 테스트를 직렬화한다.
- **정리는 자기가 만든 것만** 한다. `.harness-tmp/` 전체를 비우거나 janitor `--clean`을 실행하지 않는다(다른 에이전트의 실행 중인 환경을 부술 수 있다).
- **7절(Teardown)의 `git status`**는 원문 그대로 첨부하되, 동시 작업 중인 다른 단위의 변경분이 함께 보이면 소유 단위를 주석으로 표기하고 "이 단위가 만든 임시 아티팩트·미추적 잔여물이 없음"을 기준으로 판정한다. 전체 트리 점검은 웨이브 종료 후 오케스트레이터가 별도로 수행한다.
- **공유 문서**: `traceability.md`·`decisions.md`를 직접 수정하지 않는다. 갱신할 내용(REQ-ID, 컬럼, 값)은 결과서 하단 "공유 문서 갱신 요청" 절에 적어 반환한다.
- 결함 때문에 5단계로 되돌릴 때, 원인이 이 단위 밖(다른 단위·공유 자원·상위 산출물)에 있어 보이면 그 가능성을 결과서에 명시한다 — 병렬 단위의 규칙 F 회귀 범위 판단 근거가 된다.
- 다른 단위의 파일을 수정하지 않는다. 사소한 오탈자 수정도 이 단위의 범위 안에서만 한다.
- **06·07 병합 조건**: 병렬 호출로 이 단위를 검증할 때는 위 "06·07 병합 조건"을 적용하지 않는다(병렬 중에는 "마지막 유닛"이 정해지지 않는다). 개별 `unit-<n>-test.md`만 산출한다. 모든 단위의 06이 PASS된 뒤 오케스트레이터가 "병합 호출"로 지정한 경우에만 07 범위를 수행하고 `feature-<name>-integration-test.md`로 낸다.
