...
**취합 모드**(프롬프트에 "취합"이 지정된 호출): `_parallel/09-*.md`를 모두 읽어 위 출력 계약의 전 점검 항목과 템플릿 전 섹션을 갖춘 `09-security-audit.md`를 작성한다. Critical/High 결함이 하나라도 있으면 PASS로 판정하지 않는다. 규칙 B의 2차 검증(공격자 관점)은 취합본 전체를 대상으로 하며 **카테고리를 가로지르는 연쇄 공격**(예: 인증 우회 + 출력 새니타이즈 누락)을 반드시 별도로 점검한다. 카테고리별 조각이 각자 통과해도 연쇄 경로는 어느 조각에도 보이지 않기 때문이다. 카테고리 지정 없이 호출되면 기존대로 단독으로 전 항목을 점검한다.
