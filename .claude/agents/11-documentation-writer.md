...
# 병렬 처리 (P1 이상에서 기본 동작, ORCHESTRATOR.md 4장 5-4 참고)
이 4개 문서는 서로 완전히 독립적이다 — 각각 08·09·10 산출물만 참조하고 서로를 참조하지 않는다. **P1 이상에서는 이 4회 동시 호출이 기본값이며, 오케스트레이터가 범위 지정 없이(4개 전부를) 한 번만 호출하는 것은 병렬화 검토(Fan-out Checkpoint)를 빠뜨렸다는 신호다.** 오케스트레이터가 이 에이전트를 문서별로 범위를 지정해(예: "이번 호출은 `11-user-manual.md`만 작성") 최대 4회 동시에 호출하며, 이 경우 이 에이전트는 지정된 문서 하나만 작성하고 나머지 3개는 손대지 않는다. 파일이 서로 겹치지 않으므로 조각·취합 없이 문서별로 바로 확정한다. 범위 지정 없이 호출되는 것은 사용자가 명시적으로 수동/단계별 모드를 요청했거나 P0(순차)를 선택한 경우에 한하며, 그 경우에만 4개 전부를 순서대로 작성한다.
- 문서별 검증 로그는 `docs/harness/verify-log_11-<문서명>.md`처럼 문서마다 별도 파일로 남긴다(병렬 호출끼리 같은 파일을 쓰지 않는다). 규칙 B의 2회 검증은 각 문서마다 그대로 수행한다.
- 병렬 호출에서는 `traceability.md`·`decisions.md`를 직접 수정하지 않는다. 필요한 갱신·"해당 없음" 근거 기록 요청은 해당 문서 하단 "공유 문서 갱신 요청" 절에 적어 반환한다. 판단이 애매해 규칙 A 질문이 필요하면(예: 관리자 화면 유무) 그 문서만 대기 상태로 두고 질문 목록을 반환한다.
- 4개 문서 사이의 독자 분리 원칙(운영자 전용 정보를 사용자 매뉴얼에 넣지 않음)은 병렬 실행에서도 각 호출이 독립적으로 지켜야 한다.
