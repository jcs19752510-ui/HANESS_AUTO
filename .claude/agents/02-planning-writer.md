...
9. 작업 단위(Work Unit) 후보 목록 — 5단계 개발이 순회할 단위 초안. **반드시 아래 표 형식으로 작성한다.** 이 정보가 없으면 오케스트레이터가 독립성을 판정할 근거가 없어 병렬 실행(ORCHESTRATOR.md 1장 "병렬 실행 모드")이 전부 순차로 폴백되기 때문이다:

   | 단위 ID | 소속 feature | 커버 REQ-ID | 선행 단위 (없으면 "없음") | 예상 수정 모듈/파일 범위 | 공유 자원 접촉 (의존성 매니페스트·DB 스키마·라우터/설정 레지스트리·공용 유틸 등, 없으면 "없음") | 병렬 가능 초안 (가능/불가/불확실 + 한 줄 근거) |
   |---|---|---|---|---|---|---|

   - 이 표는 "후보"이며 3단계 설계서가 모듈 경계를 확정한다. 코드 스캔·기획 내용으로 알 수 없는 칸은 지어내지 말고 "불확실"로 쓰고 3단계가 확정하도록 남긴다(가정 목록에도 기재).
   - "선행 단위"는 데이터/상태 의존이나 기획상의 순서(예: 로그인이 있어야 마이페이지 가능)가 있을 때만 적는다. 병렬을 늘리려고 의존을 숨기지 않는다.
