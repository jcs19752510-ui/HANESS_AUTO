...
서로 무관한 다른 feature의 07(또는 그 feature의 05·06)이 동시에 돌고 있을 수 있다. 결과서의 엄격함(전 섹션, 규칙 B 2회 검증)은 단독 실행과 동일하며, 추가로 다음을 지킨다:
- 임시 아티팩트 이름에 feature 식별자를 넣고(`.harness-tmp/venv_07_<feature>/`) 자기가 만든 것만 정리한다. `.harness-tmp/` 전체를 비우거나 janitor `--clean`을 실행하지 않는다.
- 포트·DB·파일 경로는 다른 feature와 겹치지 않게 잡는다. 겹칠 수밖에 없으면 오케스트레이터에 보고해 직렬화한다.
- 7절(Teardown)의 `git status`는 원문 그대로 첨부하고, 동시 작업 중인 다른 feature의 변경분은 소유 feature를 주석으로 표기한다. 판정 기준은 "이 feature가 만든 임시 아티팩트·미추적 잔여물이 없음"이다.
- `traceability.md`·`decisions.md`는 직접 수정하지 않고, 갱신 내용을 결과서 하단 "공유 문서 갱신 요청" 절에 적어 반환한다(L1 부채 정산 근거 기록 포함).
- 통합 시점에 다른 feature와 맞닿는 문제(공유 자원, 공통 모듈)를 발견하면 임의로 봉합하지 말고 결과서에 명시한다. 이는 8단계 범위이거나 병렬 판정 오류일 수 있어 오케스트레이터가 판단한다.
