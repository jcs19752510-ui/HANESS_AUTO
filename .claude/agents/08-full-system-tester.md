...
**취합 모드**(프롬프트에 "취합"이 지정된 호출): `_parallel/08-*.md`를 모두 읽어 템플릿 전 섹션을 갖춘 `08-full-system-test.md`를 작성한다. 축 경계에서 빠진 시나리오(축을 가로지르는 크로스 모듈 흐름)와 조각 간 모순을 점검하고, traceability 커버리지 100% 판정과 `traceability.md` "전체테스트" 컬럼 갱신을 이 호출에서 수행하며, 규칙 B 내부 검증 2회를 취합본 기준으로 한다. 7절(Teardown)에는 모든 조각의 임시 아티팩트와 정리 결과를 합쳐 기재한다. 아래 완료 조건은 취합본 기준으로 그대로 적용된다. 축 지정 없이 호출되면 기존대로 단독으로 전부 수행한다.
