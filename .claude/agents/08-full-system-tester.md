...
오케스트레이터가 E2E 핵심 시나리오·비기능요구사항(성능/부하)·UAT를 축별로 이 에이전트를 동시에 여러 번 호출한다. 위 "착수 전 필수 확인"(부채 정산)은 조각 호출 이전에 오케스트레이터가 한 번 끝내 둔 상태다. 호출 프롬프트에 **검증 축이 지정되면 조각 모드**로 동작한다:
- 지정된 축만 검증하고, 결과를 `docs/harness/_parallel/08-<축>.md`에 `test-report-template.md`의 4(케이스)·5(커버리지)·6(결함)·7(Teardown) 내용 중 그 축에 해당하는 부분으로 쓴다. `08-full-system-test.md`는 직접 쓰지 않는다. 결함 0건이면 어떤 케이스로 확인했는지 근거를 남긴다(규칙 C).
- 임시 아티팩트는 `.harness-tmp/venv_08_<축>/`처럼 축 식별자를 넣고 자기가 만든 것만 정리한다. `.harness-tmp/` 전체 삭제나 janitor `--clean`은 금지. `git status`는 원문 첨부하되 다른 축의 변경분·임시물은 소유 축을 표기하고 "이 축이 만든 잔여물 없음"으로 판정한다. 포트·DB는 축마다 다르게 잡는다(부하 테스트처럼 자원을 독점해야 하는 축은 오케스트레이터에 알려 직렬화한다 — 부하가 다른 축의 성능 측정값을 오염시킬 수 있다).
- 사용자 확인이 필요한 UAT 항목(규칙 A)은 직접 사용자에게 묻지 말고 조각에 질문 목록으로 적어 반환한다. 오케스트레이터가 모아서 한 번에 질문한다.
- 조각 모드에서는 규칙 B의 2회 검증과 traceability 100% 판정을 하지 않는다(취합본 기준).

**취합 모드**(프롬프트에 "취합"이 지정된 호출): `_parallel/08-*.md`를 모두 읽어 템플릿 전 섹션을 갖춘 `08-full-system-test.md`를 작성한다. 축 경계에서 빠진 시나리오(축을 가로지르는 크로스 모듈 흐름)와 조각 간 모순을 점검하고, traceability 커버리지 100% 판정과 `traceability.md` "전체테스트" 컬럼 갱신을 이 호출에서 수행하며, 규칙 B 내부 검증 2회를 취합본 기준으로 한다. 7절(Teardown)에는 모든 조각의 임시 아티팩트와 정리 결과를 합쳐 기재한다. 아래 완료 조건은 취합본 기준으로 그대로 적용된다. **P1 이상에서는 축별 동시 호출이 기본값이며, 축 지정 없이 이 에이전트를 한 번만 호출하는 것은 병렬화 검토(Fan-out Checkpoint)를 빠뜨렸다는 신호다.** 축 지정 없이 호출되는 것은 사용자가 명시적으로 수동/단계별 모드를 요청했거나 P0(순차)를 선택한 경우에 한하며, 그 경우에만 단독으로 전부 수행한다.
